...
for (obs in observaciones){
  ...
  for(gamma in gammas){
    ...
    #
    win.graph()
      plot(1:j, vifs, type="b", xlab="Number of independent variables", ylab="VIF", lwd=2, col="blue", main=paste("n=", obs, ", gamma=", gamma, sep=""))
      abline(h=10, lwd=2, col="black")
      text(j, vifs[j], label=j, pos = 2, lwd=2)
      grid(nx = NULL, ny = NULL, lty = 2, col = "gray", lwd = 1)
      gamma_text = gamma*100
      #savePlot(paste("simulation3_Fig1_Obs", obs, "_Gamma", gamma_text, sep=""), type="pdf")
    dev.off()
  }
  #
  print(obs)
...
pes
write.table(pes, "03_p_gamma_a.txt", sep=" & ", dec=".")